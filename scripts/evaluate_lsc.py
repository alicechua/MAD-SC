...
def run_pipeline_for_word(
    ...
    initial_state = {
        "word": word,
        "word_type": "noun",  # LSC-CTD entries are all nouns
...
def main():
    ...
    log.info("Compiling MAD-SC LangGraph pipeline… (mode=%s, grounding=%s, lexicographer=%s)",
             mode,
    ...
    for i, entry in enumerate(aligned, 1):
        ...
        # Print the debate thread to console identically to evaluate_semeval.py
        ...
        if mode == "multi":
            for hist_entry in result.get("debate_history", []):
                r = hist_entry["round"]
                print(f"\n[ROUND {r}/{num_rounds} — Team Support]:\n{hist_entry.get('arg_change', '')}\n")
                print(f"[ROUND {r}/{num_rounds} — Team Refuse]:\n{hist_entry.get('arg_stable', '')}\n")
        else:
        ...
        debate_logs[word] = {
            ...
            "change_type": verdict_obj.get("change_type"),
            "causal_driver": verdict_obj.get("causal_driver"),
            "break_point_year": verdict_obj.get("break_point_year"),
            "debate_mode": mode,
            "num_rounds": num_rounds,
        }
    ...
    # Save debate_logs.json and debate_logs.md to match SemEval pipeline
    debate_logs_file = args.output_dir / "debate_logs.json"
    with open(debate_logs_file, "w", encoding="utf-8") as f:
        json.dump(debate_logs, f, indent=2, ensure_ascii=False)
    log.info("Debate transcripts saved to %s", debate_logs_file)
    
    try:
        from scripts.export_to_markdown import export_debate_to_md
        md_file = args.output_dir / "debate_logs.md"
        export_debate_to_md(str(debate_logs_file), str(md_file))
    except Exception as e:
        log.warning("Could not export Markdown: %s", e)

    print(f"\nAll results saved to: {args.output_dir}/")
    print(f"  eval_summary.json  — metrics + per-word results")
    print(f"  debate_logs.json   — full debate objects per word")
    print(f"  debate_logs.md     — readable markdown transcripts")
    print(f"  traces/            — individual pipeline outputs per word")
